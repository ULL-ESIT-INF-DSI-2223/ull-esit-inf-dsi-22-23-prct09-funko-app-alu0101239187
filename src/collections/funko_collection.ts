...
export class FunkoCollection {
    ...
    public add (new_funko: Funko): boolean {
        if (this._elements.filter((funko) => (funko.id === new_funko.id)).length > 0) {
            return false;
        }
    ...
    public update (updated_funko: Funko): boolean {
        let index = -1;
        this._elements.forEach((funko, i) => {if (funko.id === updated_funko.id) {
            index = i;
        }});
        if (index > -1) {
    ...
    public delete (funko_id: number): boolean {
        let index = -1;
        this._elements.forEach((funko, i) => {if (funko.id === funko_id) {
            index = i;
        }});
        if (index > -1) {
    ...
    public list (): string {
        let output = "--------------------------------\n";
        this._elements.forEach((funko) => output += funko.toString() + "--------------------------------\n");
        return output;
    }
